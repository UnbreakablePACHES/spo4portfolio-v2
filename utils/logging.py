...
def log_experiment_setup(logger, cfg):
    ...
    logger.log(f"Training Params: Epochs={cfg['trainer']['epochs']}, LR={cfg['optimizer']['params']['lr']}, Device={cfg['trainer'].get('device', 'cpu')}")
    logger.log(f"--------------------------------------------------")


def log_training_epoch(logger, epoch, total_epochs, avg_loss, 
                       pred_cost=None, true_cost=None, portfolio_model=None):
    """
    监控训练过程：
    1. 始终显示 Epoch 和 Loss
    ...
    # 打印格式化日志
    logger.log(f"        Epoch {epoch:03d}/{total_epochs} | Loss: {avg_loss:.6f} | Regret(Sample): {regret_msg}")